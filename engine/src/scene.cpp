...
{
	cScene::cScene(cContext* context, const std::string& name) : iObject(context), _name(name)
	{
	...
	void cScene::DestroyEntity(entity ent)
	{
		if (ent != kInvalidEntity)
			_isEntityExist->Erase(ent);
	}

	types::boolean cScene::IsEntityExist(entity ent)
	{
